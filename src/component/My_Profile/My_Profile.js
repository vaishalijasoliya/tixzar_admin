...
const ResponsiveAppBar = (props) => {
  ...
  console.log(props, "props_____________");
  ...
  const setData = async (value) => {
    ...
    if (!!accountView && accountView.status == true) {
      formik.setFieldValue("username", accountView.data.userDetails.name);
      formik.setFieldValue("email", accountView.data.userDetails.user_name);
      formik.setFieldValue("lastname", accountView.data.userDetails.email);
      formik.setFieldValue(
        "pohoneno",
        accountView.data.userDetails.phone_number
      );
      formik.setFieldValue("Gender", accountView.data.userDetails.address);

      console.log(
        accountView.data.userDetails.profile_photo,
        "is____profile____photo"
      );

      setCreateObjectURL(accountView.data.userDetails.profile_photo);
    } else {
  ...
  const onLoginPress = async () => {
    ...
    if (!!data) {
      if (data.status == true) {
        ...
        console.log(data, "is_____data_____");
        // data.currentAccount = data.data.profile_photo;
        // props.props.profile.currentAccount = data.data.profile_photo
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      email: Yup.string().max(255).required("Last Name is required"),
      lastname: Yup.string().max(255).required("Email is required"),
      pohoneno: Yup.string().max(255).required("Phone Number is required"),
